...
void LoginWidget::slot_writeSettings(){
  ...
  settings.setValue("GlobalPlan/topic",Global_Planner_Topic_box->currentText());
  settings.setValue("GlobalPlan/color",Global_Planner_Color_box->currentText());
...
void LoginWidget::readSettings(){
 ...
 QString FixedFrame = settings.value("GlobalOption/FixedFrame","map").toString();
  ...
  bool GlobalMap_enable= settings.value("GlobalMap/enable",bool(true)).toBool();
  QString GlobalMap_topic = settings.value("GlobalMap/topic",QString("/move_base/global_costmap/costmap")).toString();
  QString GlobalMap_plan = settings.value("GlobalPlan/topic",QString("/move_base/NavfnROS/plan")).toString();
  QString GlobalMap_plan_color = settings.value("GlobalPlan/color",QString("255;0;0")).toString();
  bool LocalMap_enable= settings.value("LocalMap/enable",bool(true)).toBool();
  QString LocalMap_topic = settings.value("LocalMap/topic",QString("/move_base/local_costmap/costmap")).toString();
  QString LocalMap_plan = settings.value("LocalPlan/topic",QString("/move_base/DWAPlannerROS/local_plan")).toString();
  QString LocalMap_plan_color = settings.value("LocalPlan/color",QString("0;0;255")).toString();
...
void LoginWidget::InitWidget()
{
  ...
  //header
    ...
    // 信号槽
    connect(ui->lineEditRosIp, SIGNAL(editingFinished()), this, SLOT(SltEditFinished()));
    connect(ui->lineEditRosIp, SIGNAL(returnPressed()), this, SLOT(on_btnLogin_clicked()));
    connect(ui->lineEditMasterIp, SIGNAL(returnPressed()), this, SLOT(on_btnLogin_clicked()));
}
