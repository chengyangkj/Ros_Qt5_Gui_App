...
//    读取topic的设置
    ...
    sensor_topic=topic_setting.value("topic_sensor","sensor").toString();
...
bool QNode::init() {
    ...
    sensor_sub=n.subscribe(sensor_topic.toStdString(),1000,&QNode::sensorCallback,this);
    //机器人位置话题
    ...
    {
        qDebug()<<"name:"<<QString::fromStdString(topic.name)<<"type:"<<QString::fromStdString(topic.datatype);
    }
    return true;
}
...
QMap<QString,QString> QNode::get_topic_list()
{
    ...
    for(auto topic:topic_list)
    {
        res[QString::fromStdString(topic.name)]=QString::fromStdString(topic.datatype);
    }
    return res;
}
//机器人位置话题的回调函数
...
void QNode::sensorCallback(const std_msgs::String &message_holder)
{
    qDebug()<<QString::fromStdString(message_holder.data);
//    emit sensor(message_holder.data);
}
